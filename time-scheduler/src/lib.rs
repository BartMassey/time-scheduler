...
//! schedule.improve_with_penalty(penalty_fn, None, false, Some(5));
//! ```
//!
//! ## Key Capabilities
//!
...
/// Trait for types that can compute their own penalty score.
///
/// This trait allows schedule types to implement their own penalty calculation,
/// though the more flexible approach is to use [`Schedule::improve_with_penalty`]
/// with a closure.
///
/// # Examples
///
/// ```rust
/// use time_scheduler::{Schedule, Penalty};
///
/// #[derive(Clone)]
/// struct Activity {
///     priority: usize,
/// }
///
/// struct ScheduleWithPenalty {
///     schedule: Schedule<Activity>,
/// }
///
/// impl Penalty for ScheduleWithPenalty {
///     fn penalty(&self) -> f32 {
///         self.schedule.get_unscheduled_activities()
///             .map(|a| a.priority as f32)
///             .sum()
///     }
/// }
/// ```
pub trait Penalty {
    /// Calculate and return the penalty score for this item.
    ///
    /// Lower penalties indicate better solutions. The penalty should be
    /// non-negative, with 0.0 representing a perfect solution.
    fn penalty(&self) -> f32;
}
...
impl<A: Clone> Schedule<A> {
    ...
    /// The schedule type must implement the [`Penalty`] trait.
    ///
    /// For more flexibility, consider using [`improve_with_penalty`] instead,
    /// which allows you to provide a custom penalty function as a closure.
    ///
    /// # Arguments
    ///
    /// * `nswaps` - Maximum number of swap iterations per restart. If `None`,
    ///   defaults to `2 * (total_locations)^3`
    /// * `noise` - If `true`, occasionally makes random moves to escape local optima
    ...
    /// }
    ///
    /// // Note: This example shows the interface but won't compile
    /// // because TaskSchedule doesn't implement the required traits
    /// ```
    ///
    /// [`improve_with_penalty`]: Self::improve_with_penalty
...
impl<A: Clone> Schedule<A> {
    /// Improve the schedule using a custom penalty function.
    ///
    /// This is the recommended method for optimizing schedules. It uses a hill-climbing
    /// algorithm with configurable parameters and allows you to provide a custom
    /// penalty function as a closure.
    ///
